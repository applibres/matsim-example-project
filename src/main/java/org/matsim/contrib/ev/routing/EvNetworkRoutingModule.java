...
final class EvNetworkRoutingModule implements RoutingModule {
	...
	public List<? extends PlanElement> calcRoute(RoutingRequest request) {
		...
		if (!electricFleet.getVehicleSpecifications().containsKey(evId)) {
			...
			double soc = ev.getInitialSoc(); 
			double iniCh = ev.getInitialCharge();
			...
			if (soc <= 0.25) {		
				...
				//numberOfStops = Math.floor(estimatedOverallConsumption / capacity);
				numberOfStops = Math.floor(estimatedOverallConsumption / iniCh);
			}
			...
			if (numberOfStops < 1) {
				...
				for (Map.Entry<Link, Double> e : estimatedEnergyConsumption.entrySet()) {
					...
					//RA July 2024
					if (currentConsumption > estimatedOverallConsumption*0.8) {
